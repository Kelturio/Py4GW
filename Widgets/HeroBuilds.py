...
try:
    class _GWCASkillTemplate(ctypes.Structure):
        _fields_ = [
            ("primary", ctypes.c_uint32),
            ("secondary", ctypes.c_uint32),
            ("attributes_count", ctypes.c_uint32),
            ("attribute_ids", ctypes.c_uint32 * 12),
            ("attribute_values", ctypes.c_uint32 * 12),
            ("skills", ctypes.c_uint32 * 8),
        ]


    try:
        _GWCA_GET_SKILL_TEMPLATE = load_gwca_function(
            "?GetSkillTemplate@SkillbarMgr@GW@@YA_NIAAUSkillTemplate@12@@Z",
            restype=ctypes.c_bool,
            argtypes=(ctypes.c_uint32, ctypes.POINTER(_GWCASkillTemplate)),
        )
    except Exception:
        _GWCA_GET_SKILL_TEMPLATE = None
except Exception:
    _GWCA_GET_SKILL_TEMPLATE = None
...
def _gwca_skill_template(
    agent_id: int,
) -> Optional[tuple[int, int, List[tuple[int, int]], List[int]]]:
    if not agent_id or _GWCA_GET_SKILL_TEMPLATE is None:
        return None
    try:
        template = _GWCASkillTemplate()
    except Exception:
        return None
    try:
        success = bool(
            _GWCA_GET_SKILL_TEMPLATE(
                ctypes.c_uint32(int(agent_id)), ctypes.byref(template)
            )
        )
    except Exception:
        return None
    if not success:
        return None
    primary = int(getattr(template, "primary", 0))
    secondary = int(getattr(template, "secondary", 0))
    try:
        raw_count = int(getattr(template, "attributes_count", 0))
    except Exception:
        raw_count = 0
    max_count = min(raw_count, len(getattr(template, "attribute_ids", [])))
    attributes: List[tuple[int, int]] = []
    for idx in range(max_count):
        try:
            attr_id = int(template.attribute_ids[idx])
            points = int(template.attribute_values[idx])
        except Exception:
            continue
        if attr_id <= 0 or points <= 0:
            continue
        attributes.append((attr_id, points))
    skills: List[int] = []
    for idx in range(len(getattr(template, "skills", []))):
        try:
            skills.append(int(template.skills[idx]))
        except Exception:
            skills.append(0)
    if not skills:
        skills = [0] * 8
...
def _encode_agent_template(agent_id: int, hero_slot: Optional[int], hero_member: Any | None) -> str:
    if not agent_id:
        return ""
    template_data = _gwca_skill_template(agent_id)
    if template_data is not None:
        primary, secondary, attributes, skills = template_data
        if not any(skills):
            template_data = None
