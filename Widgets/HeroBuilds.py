...
_TEMPLATE_ALPHABET = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+-"
...
HERO_ID_TO_INDEX = {hero_id: index for index, hero_id in enumerate(HERO_INDEX_TO_ID)}
...
def _profession_to_int(profession: Any) -> int:
    if profession is None:
        return 0
    for accessor in ("ToInt", "Get", "GetID"):
        method = getattr(profession, accessor, None)
        if callable(method):
            try:
                value = method()
            except Exception:
...
def _collect_agent_professions(agent_id: int) -> tuple[int, int]:
    try:
        primary, secondary = GLOBAL_CACHE.Agent.GetProfessionIDs(agent_id)
    except Exception:
        return 0, 0
    try:
...
def _collect_agent_attributes(agent_id: int) -> List[tuple[int, int]]:
    ...
    for attribute in attributes:
        ...
        if attr_id is None:
            continue
        try:
            attr_key = int(attr_id)
        except (TypeError, ValueError):
            continue
        base_points = getattr(attribute, "level_base", 0)
        if not base_points:
            base_points = getattr(attribute, "level", 0)
        try:
        ...
        previous = attribute_map.get(attr_key, 0)
        if points > previous:
            attribute_map[attr_key] = points
    return sorted(attribute_map.items())


def _bits_for_profession(max_value: int) -> int:
    bits = 4
    while (1 << bits) <= max_value and bits < 10:
        bits += 2
    if bits > 10:
        return 10
    return bits
...
def _bits_for_attribute(max_value: int) -> int:
    bits = 4
    while (1 << bits) <= max_value and bits < 4 + 15:
        bits += 1
    return bits


def _bits_for_skill(max_value: int) -> int:
    bits = 8
    while (1 << bits) <= max_value and bits < 8 + 15:
        bits += 1
    return bits


def _write_bits(buffer: List[int], value: int, bit_count: int) -> None:
    for bit_index in range(bit_count):
        buffer.append((value >> bit_index) & 1)


def _encode_skill_template_data(
    primary: int,
    secondary: int,
    ...
    bits: List[int] = []
    if len(skills) < TEAM_SIZE:
        skills = skills + [0] * (TEAM_SIZE - len(skills))
    elif len(skills) > TEAM_SIZE:
        skills = skills[:TEAM_SIZE]
    _write_bits(bits, 0xE, 4)
    _write_bits(bits, 0, 4)
    max_profession = max(primary, secondary)
    bits_per_profession = _bits_for_profession(max_profession)
    profession_code = max((bits_per_profession - 4) // 2, 0)
    _write_bits(bits, profession_code, 2)
    _write_bits(bits, primary, bits_per_profession)
    _write_bits(bits, secondary, bits_per_profession)
    attributes = attributes[:12]
    _write_bits(bits, len(attributes), 4)
    max_attribute = max((attr_id for attr_id, _ in attributes), default=0)
    bits_per_attribute = _bits_for_attribute(max_attribute)
    attribute_code = max(bits_per_attribute - 4, 0)
    _write_bits(bits, attribute_code, 4)
    for attr_id, points in attributes:
        _write_bits(bits, attr_id, bits_per_attribute)
        _write_bits(bits, min(points, 15), 4)
    max_skill = max((skill_id for skill_id in skills), default=0)
    bits_per_skill = _bits_for_skill(max_skill)
    skill_code = max(bits_per_skill - 8, 0)
    _write_bits(bits, skill_code, 4)
    for skill_id in skills:
        _write_bits(bits, skill_id, bits_per_skill)
    _write_bits(bits, 0, 1)
    while len(bits) % 6:
        bits.append(0)
    characters: List[str] = []
    for index in range(0, len(bits), 6):
        ...
        value = 0
        for bit_position, bit_value in enumerate(chunk):
            value |= (bit_value & 1) << bit_position
        characters.append(_TEMPLATE_ALPHABET[value])
    return "".join(characters)


def _skill_id_from_entry(entry: Any) -> int:
    skill = getattr(entry, "id", None)
    if skill is None:
        return 0
    raw_id = getattr(skill, "id", None)
    try:
        return int(raw_id)
    except (TypeError, ValueError):
        return 0
...
def _collect_player_skills() -> List[int]:
    skills: List[int] = []
...
def _collect_hero_skills(hero_index: int) -> List[int]:
    try:
    ...
    if not skillbar:
        return [0] * TEAM_SIZE
    skills = [0] * TEAM_SIZE
    for slot, entry in enumerate(skillbar):
        if slot >= TEAM_SIZE:
            break
        skills[slot] = _skill_id_from_entry(entry)
    return skills


def _hero_index_from_member(hero_member: Any) -> int:
    hero_object = getattr(hero_member, "hero_id", None)
    if hero_object is None:
        return 0
    try:
        hero_id_value = hero_object.GetID()
    except Exception:
        return 0
    try:
        hero_type = HeroType(hero_id_value)
    except ValueError:
        return 0
    return HERO_ID_TO_INDEX.get(hero_type, 0)


def _hero_professions(hero_member: Any, agent_id: int) -> tuple[int, int]:
    ...
    return _collect_agent_professions(agent_id)


def _add_teambuild_from_current() -> bool:
    global builds_changed, teambuilds, selected_teambuild_for_copy
    if Map.IsMapLoading():
        Py4GW.Console.Log(
            MODULE_NAME,
            "Cannot add a teambuild while the map is still loading.",
            Py4GW.Console.MessageType.Warning,
        )
        return False
    ...
    if not player_agent:
        Py4GW.Console.Log(
            MODULE_NAME,
            "Unable to read the current skill template: player agent unavailable.",
            Py4GW.Console.MessageType.Warning,
        )
        return False
    ...
    captured_any = False
    primary_prof, secondary_prof = _collect_agent_professions(player_agent)
    attributes = _collect_agent_attributes(player_agent)
    player_skills = _collect_player_skills()
    player_code = _encode_skill_template_data(primary_prof, secondary_prof, attributes, player_skills)
    if player_code:
        teambuild.builds[0].code = player_code
        captured_any = True
    heroes = GLOBAL_CACHE.Party.GetHeroes() or []
    owner_login = _get_player_login_number()
    for slot in range(1, TEAM_SIZE):
        ...
        if hero_index >= len(heroes):
            continue
        hero_member = heroes[hero_index]
        try:
            owner_id = hero_member.owner_player_id
        except AttributeError:
            owner_id = None
        if owner_login is not None and owner_id is not None and owner_id != owner_login:
            continue
        agent_id = getattr(hero_member, "agent_id", 0)
        if not agent_id:
            continue
        hero_teambuild_index = _hero_index_from_member(hero_member)
        if hero_teambuild_index <= 0:
            continue
        primary_prof, secondary_prof = _hero_professions(hero_member, agent_id)
        attributes = _collect_agent_attributes(agent_id)
        hero_skills = _collect_hero_skills(slot)
        hero_code = _encode_skill_template_data(primary_prof, secondary_prof, attributes, hero_skills)
        if not hero_code:
            continue
        build = teambuild.builds[slot]
        build.code = hero_code
        build.hero_index = hero_teambuild_index
        build.behavior = 1
        captured_any = True
    if not captured_any:
        Py4GW.Console.Log(
            MODULE_NAME,
            "No player or hero templates could be read from the current party.",
            Py4GW.Console.MessageType.Warning,
        )
        return False
    teambuilds.append(teambuild)
    selected_teambuild_for_copy = max(len(teambuilds) - 1, 0)
    builds_changed = True
    return True
...
def _draw_main_window() -> bool:
    ...
    try:
        ...
        PyImGui.show_tooltip("Capture the current player and hero builds as a new teambuild.")
        if teambuilds:
